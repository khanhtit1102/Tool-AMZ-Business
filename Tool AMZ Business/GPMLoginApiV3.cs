...
namespace GPMLoginAPISampleSeleniumAndPuppeteer.Libs
{
    ...
    {
        private string START_ENDPOINT = "/api/v3/profiles/start/{id}";

        private string _apiUrl;

        public GPMLoginApiV3(string apiUrl, string win_pos, string win_size, double win_scale = 1)
        {
            _apiUrl = apiUrl;
            if(win_pos != null && win_size != null)
            {
                START_ENDPOINT += $"?win_pos={win_pos}&win_size={win_size}&win_scale={win_scale}";
            }
        }
